...
type greetArgs struct {
	...
	Formal bool   `json:"formal,omitempty" desc:"Use a formal greeting when true."`
}
...
func TestNewTool_SchemaMatchesInputStruct(t *testing.T) {
	...
	cases := []struct {
		...
		{"type_is_function", func(t *testing.T) { assert.Equal(t, "function", def.Type) }},
		{"name", func(t *testing.T) { assert.Equal(t, "greet", def.Function.Name) }},
		{"description", func(t *testing.T) { assert.Equal(t, greetDesc, def.Function.Description) }},
		{"strict_is_true", func(t *testing.T) { assert.True(t, def.Function.Strict) }},
		...
		{"identical_to_NewStrictTool", func(t *testing.T) {
			want := schema.NewStrictTool("greet", greetDesc, greetArgs{})
...
func TestNewToolWithDefinition_UsesProvidedDefinition(t *testing.T) {
	customDef := schema.FormatToolDefinition(
		"custom_search",
		"Custom description.",
		schema.NewInputSchemaFromStruct(greetArgs{}),
		false, // non-strict
	)
...
func TestNewToolWithDefinition_Execute_Works(t *testing.T) {
	def := schema.NewStrictTool("greet", greetDesc, greetArgs{})
...
func TestNewToolWithDefinition_NonStrictMode(t *testing.T) {
	params := schema.NewInputSchemaFromStruct(greetArgs{})
	def := schema.FormatToolDefinition("greet", greetDesc, params, false)
	tool := NewToolWithDefinition(def, greetHandler)

	require.Nil(t, def.Function.Parameters.AdditionalProperties,
...
func TestNewToolWithDefinition_DefinitionNotDerivedFromStruct(t *testing.T) {
	...
	tool := NewToolWithDefinition(def, greetHandler)
	assert.Equal(t, "manual_tool", tool.Definition().Function.Name)
	assert.Equal(t, "Manually defined.", tool.Definition().Function.Description)
}
