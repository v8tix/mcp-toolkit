...
func (m mockTool) Definition() model.ToolDefinition {
	type args struct {
		Input string `json:"input" desc:"test input"`
	}
	return schema.NewStrictTool(m.toolName, m.desc, args{})
}
...
func (m *dynamicMockTool) Definition() model.ToolDefinition {
	type args struct {
		Input string `json:"input" desc:"test input"`
	}
	return schema.NewStrictTool(m.toolName, *m.desc, args{})
...
func TestRegistry_All(t *testing.T) {
	...
	cases := []struct {
		...
		{"copy_safe_mutations_do_not_affect_registry", func(t *testing.T) {
			reg := New(a)
			snapshot := reg.All()
			snapshot[0] = model.ToolDefinition{}
			assert.Equal(t, "tool_a", reg.All()[0].Function.Name)
		}},
		...
		{"multiple_tools_all_returned_in_insertion_order", func(t *testing.T) {
			...
			assert.Equal(t, "tool_a", defs[0].Function.Name)
			assert.Equal(t, "tool_b", defs[1].Function.Name)
		}},
...
func TestRegistry_ByName(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.query, func(t *testing.T) {
			...
			if tc.wantOk {
				assert.Equal(t, tc.wantName, tool.Definition().Function.Name)
			}
		})
	}
}
...
func TestRegistry_Deduplication_ReplaceSemantics(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, tc.wantDesc, defs[tc.wantDescIdx].Function.Description)
		})
	}
}
...
func TestRegistry_DefinitionCalledAtReadTime(t *testing.T) {
	...
	require.Equal(t, "version 1", r.All()[0].Function.Description)
	desc = "version 2"
	assert.Equal(t, "version 2", r.All()[0].Function.Description)
...
func TestRegistry_Remove_ThenAdd_WorksCorrectly(t *testing.T) {
	...
	defs := r.All()
	require.Len(t, defs, 1)
	assert.Equal(t, "re-added", defs[0].Function.Description)
}
...
func TestRegistry_Filter(t *testing.T) {
	cases := []struct {
		...
		wantNames      []string
		checkOrigNames []string // non-nil: assert original registry unchanged
	}{
		{
		...
		},
		{
			...
			fn: func(t model.Tool) bool {
				return t.Definition().Function.Name == "alpha"
			},
			wantNames:      []string{"alpha"},
