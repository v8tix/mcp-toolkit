...
func (t nonExecutableTool) Definition() *sdkmcp.Tool {
	type args struct {
		Input string `json:"input" description:"input"`
	}
	return schema.StrictTool(t.name, "non-executable", args{})
}
...
func TestRegisterTools_Filter(t *testing.T) {
	notInternal := func(t model.Tool) bool {
		return t.Definition().Name[:8] != "internal"
	}
	notSearch := func(t model.Tool) bool {
		return t.Definition().Name != "internal_search"
	}
	...
	cases := []struct {
		...
		},
		{
			...
			filters:   []func(model.Tool) bool{func(t model.Tool) bool { return t.Definition().Name != "farewell" }},
			wantCount: 1,
			wantNames: []string{"greet"},
		},
		{
