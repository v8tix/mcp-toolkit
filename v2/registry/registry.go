...
// Panics if any tool's Definition().Name is empty — an empty name is always a
// programmer error: the LLM API rejects tools without a name, and a nameless
// tool cannot be dispatched by ByName.
...
// All returns the tool definitions of every registered tool in insertion order.
...
func (r *Registry) All() []*sdkmcp.Tool {
	r.mu.RLock()
	defer r.mu.RUnlock()
	defs := make([]*sdkmcp.Tool, len(r.tools))
...
// ByName looks up a tool by its name and returns the model.Tool.
