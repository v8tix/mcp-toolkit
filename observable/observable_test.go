...
func TestWithRetryPolicy_CustomPolicyUsed(t *testing.T) {
	...
	tool := observable.New("t", "t", fn, observable.WithRetryPolicy(fixedRetryPolicy{max: 5}))
...
func TestWithErrorPolicy_PermanentPolicyStopsRetry(t *testing.T) {
	...
	tool := observable.New("t", "t", fn,
		...
		observable.WithErrorPolicy(permanentPolicy{}),
	)
...
func TestWithClassifier_PermanentErrorStopsImmediately(t *testing.T) {
	...
	tool := observable.New("t", "t", fn,
		observable.WithMaxRetries(5),
		observable.WithClassifier(func(err error) error { return backoff.Permanent(err) }),
	)
...
func TestExecuteRx_ZeroMaxRetries_TriesOnce(t *testing.T) {
	...
	tool := observable.New("t", "t", fn, observable.WithMaxRetries(0))
...
func TestExecuteRx_TransientError_RetriesAndSucceeds(t *testing.T) {
	...
	tool := observable.New("greet", "Greet.", fn, observable.WithMaxRetries(5))
...
func TestExecuteRx_ExhaustedRetries_EmitsError(t *testing.T) {
	...
	tool := observable.New("t", "t", fn, observable.WithMaxRetries(2))
...
func TestCustomOption_ExternalPackageCanDefineNewOption(t *testing.T) {
	...
	tool := observable.New("t", "t", fn, withFixedRetries(2))
...
func TestOptionsComposition_LaterOptionOverridesEarlier(t *testing.T) {
	...
	opts := append(observable.DefaultOptions(), observable.WithMaxRetries(1))
	tool := observable.New("t", "t", fn, opts...)
...
func TestPermanent_InHandlerStopsRetry(t *testing.T) {
	...
	tool := observable.New("t", "t", fn, observable.WithMaxRetries(5))
...
func TestExecuteRx_ConcurrentCalls_IndependentBackoff(t *testing.T) {
	...
	toolA := observable.New("a", "a", fnA, observable.WithRetryPolicy(fixedRetryPolicy{max: 5}))
	toolB := observable.New("b", "b", fnB, observable.WithRetryPolicy(fixedRetryPolicy{max: 5}))
...
func TestWrap_RetryCountMatchesNew(t *testing.T) {
	...
	toolNew := observable.New("t", "t", fnNew, observable.WithMaxRetries(2))
	toolWrap := observable.Wrap(
		handler.NewTool("t", "t", fnWrap),
		observable.WithMaxRetries(2),
	)
...
func TestWithRetryPolicy_Nil_AppliesDefault(t *testing.T) {
	...
	assert.NotPanics(t, func() {
		tool := observable.New("t", "t", fn, observable.WithRetryPolicy(nil))
...
func TestWithErrorPolicy_Nil_AppliesDefault(t *testing.T) {
	...
	assert.NotPanics(t, func() {
		tool := observable.New("t", "t", fn, observable.WithErrorPolicy(nil))
		result, err := tool.Execute(context.Background(), []byte(`{"name":"x"}`))
		assert.NoError(t, err)
		assert.Equal(t, "ok", result)
	})
...
func TestExecuteRx_ContextCancellation_StopsRetry(t *testing.T) {
	...
	tool := observable.New("t", "t", fn,
		observable.WithMaxRetries(10), // large budget — cancellation must cut it short
	)
...
func TestWithOnRetry_CalledOnTransientFailure(t *testing.T) {
	...
	tool := observable.New("t", "d",
		...
		observable.WithMaxRetries(3),
		observable.WithOnRetry(func(attempt uint64, _ error) {
...
func TestWithOnRetry_NotCalledOnPermanentError(t *testing.T) {
	...
	tool := observable.New("t", "d",
		...
		observable.WithMaxRetries(3),
		observable.WithOnRetry(func(_ uint64, _ error) { hookCalled = true }),
	)
